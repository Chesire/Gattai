...
class SearchAggregatorTests {
    ...
    private fun params() = SearchQuery("test", SeriesType.ANIME)
    ...
    fun `kitsuIdOnly manga returns single series with kitsuId`() {
        every { searchService.search(any()) } returns SearchServiceResult.Success(
            listOf(
    ...
    fun `twoServices sharedMalId merged into one entry with all ids`() {
        ...
        every { searchService2.search(any()) } returns SearchServiceResult.Success(
            listOf(
